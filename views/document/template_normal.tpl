...
RunDoc 的前身是 SmartWiki 文档系统。SmartWiki 是基于 PHP 框架 laravel 开发的一款文档管理系统。因 PHP 的部署对普通用户来说太复杂，所以改用 Golang 开发。可以方便用户部署和实用。
...
**Windows 教程:** [https://github.com/xqk/rundoc/blob/master/README_WIN.md](docs/README_WIN.md)

**Linux  教程:**  [https://github.com/xqk/rundoc/blob/master/README_LINUX.md](docs/README_LINUX.md)

**PDF 导出配置教程**  [https://github.com/xqk/rundoc/blob/master/docs/README_LINUX.md](docs/WKHTMLTOPDF.md)
...
RunDoc 使用MySQL储存数据，且编码必须是`utf8mb4_general_ci`。请在安装前，把数据库配置填充到项目目录下的 conf/app.conf 中。

如果conf目录下不存在 app.conf 请重命名 app.conf.example 为 app.conf。

如果 RunDoc 根目录下存在 install.lock 文件表示已经初始化过数据库，想要重新初始化数据库，只需要删除该文件重新启动程序即可。
...
使用 supervisor 做服务，可以使服务器重启后自动重启 RunDoc。

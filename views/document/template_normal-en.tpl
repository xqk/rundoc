...
[![Build Status](https://travis-ci.org/lifei6671/rundoc.svg?branch=master)](https://travis-ci.org/lifei6671/rundoc)

RunDoc is a simple and easy-to-use document management system developed for IT teams.

RunDoc The predecessor of Mindoc was the SmartWiki documentation system. SmartWiki is a document management system developed based on the PHP framework laravel. Because the deployment of PHP is too complicated for ordinary users, it is convenient for users to deploy and use Golang instead.
...
**If the golang is not installed on your server, please manually set an environment variable as follows: the key name is ZONEINFO, and the value is RunDoc and /lib/time/zoneinfo.zip in the directory.**

**Windows tutorial:** [https://github.com/xqk/rundoc/blob/master/README_WIN.md](docs/README_WIN.md)

**Linux  tutorial:**  [https://github.com/xqk/rundoc/blob/master/README_LINUX.md](docs/README_LINUX.md)

**PDF Export configuration tutorial**  [https://github.com/xqk/rundoc/blob/master/docs/README_LINUX.md](docs/WKHTMLTOPDF.md)

For users without Golang experience, you can download the compiled program from here. [https://github.com/xqk/rundoc/releases](https://github.com/xqk/rundoc/releases) 
...
RunDoc uses MySQL to store data, and the encoding must be `utf8mb4_general_ci`.
...
Using supervisor as a service can automatically restart RunDoc after the server restarts.
...
```bash
# Create slave.vbs file in the RunDoc root directory:
...
Please add RunDoc to the boot list.
...
```bash
docker run -p 8181:8181 -e MYSQL_PORT_3306_TCP_ADDR=127.0.0.1 -e MYSQL_PORT_3306_TCP_PORT=3306 -e MYSQL_INSTANCE_NAME=rundoc_db -e MYSQL_USERNAME=root -e MYSQL_PASSWORD=123456 -e httpport=8181 -d daocloud.io/lifei6671/rundoc:latest
```

...
# 工作目录
ADD . /go/src/github.com/xqk/rundoc
WORKDIR /go/src/github.com/xqk/rundoc

# 编译
RUN go env
RUN go mod tidy -v
RUN go build -v -o rundoc_linux_amd64 -ldflags "-w -s -X 'main.VERSION=$TAG' -X 'main.BUILD_TIME=`date`' -X 'main.GO_VERSION=`go version`'"
...
# 测试编译的rundoc是否ok
RUN ./rundoc_linux_amd64 version

# 必要的文件复制
ADD simsun.ttc /usr/share/fonts/win/
ADD start.sh /go/src/github.com/xqk/rundoc
...
COPY --from=build /go/src/github.com/xqk/rundoc/rundoc_linux_amd64 /rundoc/
COPY --from=build /go/src/github.com/xqk/rundoc/start.sh /rundoc/
COPY --from=build /go/src/github.com/xqk/rundoc/LICENSE.md /rundoc/
# 文件夹复制
COPY --from=build /go/src/github.com/xqk/rundoc/lib /rundoc/lib
COPY --from=build /go/src/github.com/xqk/rundoc/conf /rundoc/__default_assets__/conf
COPY --from=build /go/src/github.com/xqk/rundoc/static /rundoc/__default_assets__/static
COPY --from=build /go/src/github.com/xqk/rundoc/views /rundoc/__default_assets__/views
COPY --from=build /go/src/github.com/xqk/rundoc/uploads /rundoc/__default_assets__/uploads
...
VOLUME ["/rundoc/conf","/rundoc/static","/rundoc/views","/rundoc/uploads","/rundoc/runtime","/rundoc/database"]

# refer: https://docs.docker.com/engine/reference/builder/#expose
EXPOSE 8181/tcp

ENV ZONEINFO=/rundoc/lib/time/zoneinfo.zip
RUN chmod +x /rundoc/start.sh

ENTRYPOINT ["/bin/bash", "/rundoc/start.sh"]

# https://docs.docker.com/engine/reference/commandline/build/#options
# docker build --progress plain --rm --build-arg TAG=2.1 --tag gsw945/rundoc:2.1 .
# https://docs.docker.com/engine/reference/commandline/run/#options
# set RUNDOC=//d/rundoc # windows
# export RUNDOC=/home/ubuntu/rundoc-docker # linux
# docker run -d --name=rundoc --restart=always -v /www/rundoc/uploads:/rundoc/uploads -v /www/rundoc/database:/rundoc/database  -v /www/rundoc/conf:/rundoc/conf  -e RUNDOC_DB_ADAPTER=sqlite3 -e RUNDOC_DB_DATABASE=./database/rundoc.db -e RUNDOC_CACHE=true -e RUNDOC_CACHE_PROVIDER=file -p 8181:8181 rundoc-org/rundoc:v2.1
